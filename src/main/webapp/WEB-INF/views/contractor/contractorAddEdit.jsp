...
<c:if test="${operation == 'display'}"> <h1>Display contractor</h1></c:if>
...
<form:form method="post" modelAttribute="contractor">
    <fieldset <c:if test="${operation == 'display'}"> disabled = "disabled"</c:if>>
        ...
        Contractor type: <form:select itemValue="id" itemLabel="description" path="contractorType.id" items="${contractorTypes}"/> <form:errors path="contractorType.id"/><br>
        First name: <form:input type="text" path="transactionParty.firstName"/> <form:errors path="transactionParty.firstName"/><br>
        SecondName: <form:input type="text" path="transactionParty.secondName"/> <form:errors path="transactionParty.secondName"/><br>
    ...
    <c:if test="${!(operation == 'display')}"> <br><input class="btn btn-secondary btn" type="submit" value="Submit"></c:if>
</form:form>
    <c:if test="${operation == 'display'}">
        <a class="btn btn-secondary btn" href="../list">Go back</a>
    </c:if>
